...
func _on_hitbox_body_entered(body):
	...
	if body.type == 'bullet':
		$AnimationTree.set("parameters/OneShot_idle/active", false)
		$AnimationTree.set("parameters/OneShot_hurt/active", true)
		Game.playerhealth -= Game.enemy_data[body.turrettype]['damage']
		Globals.camera.shake(2500, 0.1 ,1500)
		body.queue_free()
		$Timer.start(.3); yield($Timer, "timeout")
		$AnimationTree.set("parameters/OneShot_hurt/active", false)
...
func shoot():
	...
	bullet.rotation = Vector2(1, 0).angle_to((mousePos - position).normalized())
